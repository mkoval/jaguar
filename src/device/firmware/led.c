...
LEDTick(void)
{
    ...
    //
    // See if a fault has occurred.  Only make this transition if not in the
    // delay state (do not preempt a status indication with a fault
    // indication).
    //
    if((ulFaults != 0) && (g_eLEDState != LED_STATE_LINK_BAD) &&
